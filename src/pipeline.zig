    ...
    return switch (step_spec.step) {
        ...
        .bevel_edge => {
            if (pickFirstSharedEdge(mesh)) |edge| {
                ...
                return mesh_bevel_edge.bevelEdges(allocator, mesh, &edges, .{
                    .width = step_spec.bevel_edge_width orelse 0.12,
                });
            }
        ...
        .fill_hole => mesh_fill_hole.fillHole(allocator, mesh, .{}),
    ...
    if (params_text) |raw_params| {
        ...
        while (params.next()) |assignment| {
            ...
            switch (spec.step) {
                ...
                .bevel_edge => {
                    if (std.mem.eql(u8, key, "width")) {
                        spec.bevel_edge_width = try parseFloat(value_text);
                    } else {
                        return error.UnsupportedPipelineParameter;
                    }
                },
...
test "pipeline can parse transform and array steps" {
    ...
    const fill_hole = try parseStepSpec("fill-hole");
    ...
    try std.testing.expectEqual(Step.fill_hole, fill_hole.step);
...
test "pipeline can fill one loose hole loop after face deletion" {
    const steps = [_]StepSpec{
        .{ .step = .delete_face, .delete_face_index = 0 },
        .{ .step = .fill_hole },
    };
...
test "pipeline can bevel one shared grid edge" {
    const steps = [_]StepSpec{
        .{ .step = .bevel_edge, .bevel_edge_width = 0.12 },
    };
