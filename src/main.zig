...
fn buildDerivedMeshCommand(allocator: std.mem.Allocator, command: []const u8) !blendzig.mesh.Mesh {
    ...
    if (std.mem.eql(u8, command, "mesh-bevel-edge")) {
        var source_mesh = try createBevelHingeMesh(allocator);
        defer source_mesh.deinit();
    ...
    if (std.mem.eql(u8, command, "mesh-delete-face")) {
        ...
        defer source_mesh.deinit();
        return blendzig.geometry.deleteFaces(allocator, &source_mesh, &[_]usize{0}, .{});
    }
    if (std.mem.eql(u8, command, "mesh-fill-hole")) {
        var source_mesh = try blendzig.geometry.createGridMesh(allocator, 2, 2, 2.0, 2.0, true);
        defer source_mesh.deinit();
        var deleted = try blendzig.geometry.deleteFaces(allocator, &source_mesh, &[_]usize{0}, .{});
        defer deleted.deinit();
        return blendzig.geometry.fillHole(allocator, &deleted, .{});
    }
    ...
    if (std.mem.eql(u8, command, "mesh-inset-region")) {
        var source_mesh = try blendzig.geometry.createGridMesh(allocator, 3, 2, 2.0, 1.0, true);
        defer source_mesh.deinit();
        return blendzig.geometry.insetRegion(allocator, &source_mesh, .{ .width = 0.2 });
    }
...
fn createBevelHingeMesh(allocator: std.mem.Allocator) !blendzig.mesh.Mesh {
    var mesh = try blendzig.mesh.Mesh.init(allocator);
    errdefer mesh.deinit();

    // This folded hinge keeps the bevel visually obvious: the shared edge sits between
    // one flat quad and one vertical quad, so the new strip reads as a real chamfer.
...
test "mesh bevel edge command replaces the hinge seam with a chamfer strip" {
    var mesh = try buildDerivedMeshCommand(std.testing.allocator, "mesh-bevel-edge");
    defer mesh.deinit();

    try std.testing.expectEqual(@as(usize, 10), mesh.vertexCount());
...
test "mesh delete face command opens a hole and keeps the deleted border wire" {
    var mesh = try buildDerivedMeshCommand(std.testing.allocator, "mesh-delete-face");
    defer mesh.deinit();
...
test "mesh fill hole command restores a deleted single face loop" {
    var mesh = try buildDerivedMeshCommand(std.testing.allocator, "mesh-fill-hole");
    defer mesh.deinit();
...
test "mesh inset region command builds one inset cap and outer rim" {
    var mesh = try buildDerivedMeshCommand(std.testing.allocator, "mesh-inset-region");
    defer mesh.deinit();
